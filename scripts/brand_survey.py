...
The deflection rule itself (normalize, redirect patterns, residual stripping,
MAX_RESIDUAL_WORDS) has moved to src/deflection.py, because src/sample.py needs
it and a pipeline stage must not import from a one-off script. The move was
verbatim; this script's output is unchanged by it.
